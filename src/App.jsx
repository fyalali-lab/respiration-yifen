...
export default function VitalCoach() {
  ...
  // --- CLIENT FORM ---
  const [clientForm, setClientForm] = useState({ name: "", age: 35, sex: "M", weight: 75, height: 175, activity: "moderate", smoker: false, medication: "", objectives: ["stress"], context: [], consent: false });
    ...
    resonanceTimerRef.current = setInterval(() => {
      setResonanceTimer(p => {
        if (p + 1 >= RESONANCE_STEP_DURATION) {
          ...
          // Calculate RMSSD for this step
          const stepRmssd = demoMode
            ? 18 + Math.random() * 20 + (step === 2 ? 8 : 0) // Demo: peak at 5.5/min
            : HRV.rmssd(rrData);
          const stepSdnn = demoMode ? 25 + Math.random() * 15 : HRV.sdnn(rrData);
          setResonanceResults(prev => [...prev, {
  ...
  const finishResonance = () => {
    ...
    // Find best frequency (highest RMSSD)
    const best = resonanceResults.reduce((a, b) => a.rmssd > b.rmssd ? a : b);
    const halfCycle = best.cycleDuration / 2;
    // Update client with custom resonance
    const updatedClient = {
      ...activeClient,
  ...
  const generatePDF = () => {
    ...
    a.href = url;
    a.download = `VitalCoach_${client.name.replace(/\s/g, "_")}_${new Date().toISOString().slice(0, 10)}.txt`;
    a.click();
          ...
          <div>
            <div style={{ textAlign: "center", marginBottom: "32px" }}>
              <h1 style={{ fontFamily: T.fontDisplay, fontSize: "26px", fontWeight: 800, letterSpacing: "-0.5px", margin: "0 0 6px" }}>
                {activeClient ? `Bonjour, ${activeClient.name.split(" ")[0]}` : "VitalCoach"}
              </h1>
            ...
            {activeClient && (
              <Card style={{ marginBottom: "16px", border: `1px solid ${T.accent}25` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontFamily: T.fontDisplay, fontWeight: 600, fontSize: "16px" }}>{activeClient.name}</div>
                  ...
                  <div style={{ display: "flex", gap: "6px" }}>
                    <Btn variant="ghost" onClick={() => setView("resonance")} style={{ fontSize: "12px" }}>🎯 Résonance</Btn>
                ...
                {activeClient.resonanceFreq && (
                  <div style={{ marginTop: "8px", padding: "6px 12px", borderRadius: "8px", background: `${T.good}08`, border: `1px solid ${T.good}20`, fontSize: "11px", color: T.good }}>
            ...
            {!activeClient && (
              <div style={{ marginBottom: "20px" }}>
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  ...
                  <Btn variant="secondary" onClick={() => { setDemoMode(true); setActiveClient({ id: "demo", name: "Client Démo", age: 35, sex: "M", weight: 75, height: 175, bmi: "24.5", activity: "moderate" }); }}>
                    Mode démo
                  </Btn>
                </div>
              </div>
        ...
        {view === "newClient" && (
          <div>
            ...
            <Card style={{ marginBottom: "12px" }}>
              ...
              <input placeholder="Prénom Nom" value={clientForm.name} onChange={e => setClientForm(p=>({...p, name: e.target.value}))}
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              <Btn variant="success" full disabled={!clientForm.name || !clientForm.consent} onClick={saveClient}>Créer le profil</Btn>
            </div>
          </div>
        )}
          ...
          <div>
            <div style={{ textAlign: "center", marginBottom: "20px" }}>
              <h2 style={{ fontFamily: T.fontDisplay, fontSize: "20px", fontWeight: 700, margin: "0 0 4px" }}>
                Bilan de séance {activeClient ? `— ${activeClient.name}` : ""}
              </h2>
            ...
            <Card style={{ marginBottom: "12px" }}>
              ...
              {preMetrics && postMetrics && (() => {
                const d = ((postMetrics.rmssd - preMetrics.rmssd)/preMetrics.rmssd)*100;
                if (d > 15) return <div style={{ fontSize: "13px", color: T.good }}>✅ Réponse positive significative. Poursuite recommandée avec progression.</div>;
                if (d > 5) return <div style={{ fontSize: "13px", color: T.warn }}>⚡ Amélioration modérée. Ajuster protocole ou durée.</div>;
                return <div style={{ fontSize: "13px", color: T.bad }}>📋 Réponse limitée. Envisager un autre protocole ou orienter vers un professionnel.</div>;
              })()}
        ...
        {view === "resonance" && (
          <div>
                ...
                <div style={{ display: "flex", gap: "4px", marginBottom: "12px" }}>
                  {resonanceFreqs.map((f, i) => (
                    <div key={i} style={{
                      ...
                      {i < resonanceResults.length ? `${resonanceResults[i].rmssd.toFixed(0)}ms` : `${f}/min`}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {!resonanceActive && resonanceResults.length === 5 && (
              <div>
                <Card style={{ marginBottom: "16px" }}>
                  <div style={{ fontSize: "11px", color: T.textMuted, textTransform: "uppercase", letterSpacing: "1px", marginBottom: "12px" }}>Résultats par fréquence</div>
                  
                  {/* Bar chart */}
                  <div style={{ display: "flex", alignItems: "end", gap: "8px", height: "120px", marginBottom: "12px" }}>
                    {resonanceResults.map((r, i) => {
                      const maxR = Math.max(...resonanceResults.map(x => x.rmssd));
                      const isBest = r.rmssd === maxR;
                      return (
                        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
                          <span style={{ fontFamily: T.fontMono, fontSize: "11px", color: isBest ? T.good : T.textMuted, fontWeight: isBest ? 700 : 400 }}>
                            {r.rmssd.toFixed(1)}
                          </span>
                          <div style={{
                            width: "100%", height: `${(r.rmssd/maxR)*80}px`,
                            background: isBest ? `linear-gradient(to top, ${T.good}60, ${T.good})` : `linear-gradient(to top, ${T.accent}30, ${T.accent}80)`,
                            borderRadius: "4px 4px 0 0", minHeight: "8px",
                            border: isBest ? `2px solid ${T.good}` : "none",
                          }} />
                          <span style={{ fontFamily: T.fontMono, fontSize: "10px", color: isBest ? T.good : T.textDim }}>
                            {r.breathsPerMin}/min
                          </span>
                        </div>
                      );
                  ...
                  {(() => {
                    const best = resonanceResults.reduce((a, b) => a.rmssd > b.rmssd ? a : b);
                    return (
                      <div style={{ padding: "12px 16px", borderRadius: "10px", background: `${T.good}08`, border: `1px solid ${T.good}20` }}>
                        <div style={{ fontSize: "14px", fontWeight: 600, color: T.good, marginBottom: "4px" }}>
                          🎯 Fréquence de résonance identifiée : {best.breathsPerMin} resp/min
                        </div>
                        <div style={{ fontSize: "12px", color: T.textMuted }}>
                          RMSSD maximal à {best.rmssd.toFixed(1)} ms • Cycle : {(best.cycleDuration/2).toFixed(1)}s inspire / {(best.cycleDuration/2).toFixed(1)}s expire
                        </div>
                      </div>
                    );
                  })()}
